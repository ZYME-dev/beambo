"""Test cantilever beam (fixed-free) against analytical formulas."""
...
from app.formulas import cantilever
...
class TestUniformLight:
    ...
    def setup(self):
        case = BeamCase(
            ...
            support_right=SupportType.FREE,
            section=SECTION,
    ...
    def test_shear(self):
        expected = cantilever.uniform_load_shear(self.W, L, self.x)
        assert_allclose(self.results.V[self.combo], expected, rtol=0.02, name="V")

    def test_moment(self):
        expected = cantilever.uniform_load_moment(self.W, L, self.x)
        assert_allclose(self.results.M[self.combo], expected, rtol=0.02, name="M")

    def test_deflection(self):
        expected = cantilever.uniform_load_deflection(self.W, L, self.E, self.I, self.x)
        assert_allclose(
...
class TestTipLoadLight:
    """Light tip load: P = -3 kN."""
    ...
    def setup(self):
        case = BeamCase(
            ...
            support_right=SupportType.FREE,
            section=SECTION,
            point_loads=[PointLoad(P=self.P, x=L, case=LoadCase.G)],
            n_points=N_POINTS,
        )
        self.results = compute_beam(case)
        self.x = self.results.x
    ...
    def test_shear(self):
        expected = cantilever.tip_load_shear(self.P, L, self.x)
        assert_allclose(self.results.V[self.combo], expected, rtol=0.02, name="V")

    def test_moment(self):
        expected = cantilever.tip_load_moment(self.P, L, self.x)
        assert_allclose(self.results.M[self.combo], expected, rtol=0.02, name="M")

    def test_deflection(self):
        expected = cantilever.tip_load_deflection(self.P, L, self.E, self.I, self.x)
        assert_allclose(
            self.results.delta[self.combo], expected, rtol=0.02, name="delta"
        )


class TestTipLoadHeavy:
    """Heavy tip load: P = -20 kN."""
...
class TestPointLoadMidspan:
    """Point load at midspan: P = -7 kN at L/2."""
    ...
    def test_shear(self):
        expected = cantilever.point_load_shear(self.P, self.A, L, self.x)
        assert_allclose(self.results.V[self.combo], expected, rtol=0.02, name="V")

    def test_moment(self):
        expected = cantilever.point_load_moment(self.P, self.A, L, self.x)
        assert_allclose(self.results.M[self.combo], expected, rtol=0.02, name="M")

    def test_deflection(self):
        expected = cantilever.point_load_deflection(
...
class TestPointLoadQuarterSpan:
    """Point load near fixed end: P = -10 kN at L/4."""
...
class TestPointLoadThreeQuarterSpan:
    """Point load near free end: P = -6 kN at 3L/4."""
