"""Test simply supported beam (pinned-roller) against analytical formulas."""
...
from app.formulas import simply_supported
...
class TestUniformLight:
    ...
    def test_shear(self):
        expected = simply_supported.uniform_load_shear(self.W, L, self.x)
        assert_allclose(self.results.V[self.combo], expected, rtol=0.02, name="V")

    def test_moment(self):
        expected = simply_supported.uniform_load_moment(self.W, L, self.x)
        assert_allclose(self.results.M[self.combo], expected, rtol=0.02, name="M")

    def test_deflection(self):
        expected = simply_supported.uniform_load_deflection(
...
class TestPointLoadMidspan:
    ...
    def test_shear(self):
        expected = simply_supported.point_load_shear(self.P, self.A, L, self.x)
        assert_allclose(self.results.V[self.combo], expected, rtol=0.02, name="V")

    def test_moment(self):
        expected = simply_supported.point_load_moment(self.P, self.A, L, self.x)
        assert_allclose(self.results.M[self.combo], expected, rtol=0.02, name="M")

    def test_deflection(self):
        expected = simply_supported.point_load_deflection(
